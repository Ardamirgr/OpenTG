...
uint16_t mac_output = 0;                                             /* PI controller output driving MAC valve */
...
// PI gains in fixed-point timer-counts per mbar.
// Divide final P and I terms by 100.
const int16_t boost_kp_x100 = 100;   // 1 PWM timer counts per mbar
const int16_t boost_ki_x100 = 2;     // 0.02 PWM timer counts per mbar per control step

const int32_t boost_i_max = 30000;   // integrator clamp, in accumulated mbar-error units
...
void setup() {
  ...
  ******************/
  ...
  void start() {
    ...
    if (ambient_pressure < AMBIENT_MIN_MBAR || ambient_pressure > AMBIENT_MAX_MBAR) {
      status_flags |= STATUS_AMB_FAULT;
    }
    else {
      ambient_valid = true;
      status_flags &= ~STATUS_AMB_FAULT;
  ...
  void loop() {
    ...
    if (flags & TICK_FLAG_BOOST) {
      if (map_sensor_fault || !ambient_valid) {
        boost_setpoint = 0;
        mac_output = 0;
        integ_acc = 0;
        setBoostLevel(0);
        ...
        int16_t boost_mbar = (int16_t)map_mbar - (int16_t)ambient_pressure;
        if (boost_mbar < 0) boost_mbar = 0;
        boost_setpoint = lookup_boost_setpoint(gear, rpm);
        mac_output = boost_control(boost_setpoint, (uint16_t)boost_mbar, rpm);
...
uint16_t boost_control(uint16_t boost_setpoint, uint16_t current_boost, uint16_t RPM) {
  ...
  if (RPM < BOOST_CONTROL_MIN_RPM || boost_setpoint == 0) {
    integ_acc = 0;
    overboost_latched = false;
    status_flags &= ~STATUS_OVERBOOST;
    return 0;
  }
  ...
  /*
    ...
    Because your MAC valve increases boost as duty rises:
      - safe state is 0 PWM
      - overboost must force 0 PWM
      - integrator must be cleared
  */
  if (current_boost >= boost_setpoint + OVERBOOST_CUT_MBAR) {
    integ_acc = 0;
    overboost_latched = true;
    status_flags |= STATUS_OVERBOOST;
    return 0;
  }
  ...
  /*
    ...
    Until boost falls below clear threshold, keep MAC duty at 0.
  */
  if (overboost_latched) {
    if (current_boost <= boost_setpoint + OVERBOOST_CLEAR_MBAR) {
      overboost_latched = false;
      status_flags &= ~STATUS_OVERBOOST;
    } else {
      integ_acc = 0;
  ...
  int16_t error = (int16_t)boost_setpoint - (int16_t)current_boost;

  int32_t p_term = (int32_t)boost_kp_x100 * error;

  int32_t candidate_integ = integ_acc + error;

  if (candidate_integ > boost_i_max) candidate_integ = boost_i_max;
  if (candidate_integ < -boost_i_max) candidate_integ = -boost_i_max;

  int32_t i_term = (int32_t)boost_ki_x100 * candidate_integ;

  int32_t raw_output = (p_term + i_term) / 100;

  bool saturated_high = raw_output > ICR1;
  bool saturated_low  = raw_output < 0;

  /*
    Conditional anti-windup.

    If output is already at max, only allow the integrator to move downward.
    If output is already at zero, only allow the integrator to move upward.
  */
  if ((!saturated_high || error < 0) &&
      (!saturated_low  || error > 0)) {
    integ_acc = candidate_integ;
  }

  i_term = (int32_t)boost_ki_x100 * integ_acc;
  raw_output = (p_term + i_term) / 100;

  if (raw_output > ICR1) raw_output = ICR1;
  if (raw_output < 0) raw_output = 0;

  if (MAC_MIN_EFFECTIVE_DUTY > 0 && raw_output > 0) {
    uint16_t min_pwm = ((uint32_t)ICR1 * MAC_MIN_EFFECTIVE_DUTY) / 10000;
    if (raw_output < min_pwm) raw_output = min_pwm;
  }

  return (uint16_t)raw_output;
}
...
uint16_t lookup_boost_setpoint(uint8_t gear, uint16_t rpm) {  
  ...
  const uint16_t *map_base = (const uint16_t*)pgm_read_ptr(&boost_profiles[active_profile]);
...
uint16_t read_map() {
  ...
  uint16_t map_adc_high_pressure = pgm_read_word_near(&map_x_axis[0]); // 921
  uint16_t map_adc_low_pressure  = pgm_read_word_near(&map_x_axis[1]); // 27

  if (input_adc <= map_adc_low_pressure || input_adc >= map_adc_high_pressure) {
    map_sensor_fault = true;
    status_flags |= STATUS_MAP_FAULT;
    return 0;
  }
...
uint8_t calculate_gear(uint16_t rpm, uint8_t vss) {
    if (vss < 5 || rpm < 1000) return 0; // Stopped or clutch in
    ...
    for (uint8_t i = 1; i <= VEHICLE_NUM_GEARS; i++) {
...
void setBoostLevel(uint16_t level) {
   if (level>ICR1) level = ICR1;
   OCR1A = level;   
}
...
uint8_t mac_duty_percent(void) {
  return (uint8_t)(((uint32_t)mac_output * 100) / ICR1);
}
