...
void main_Menu() {
  ...
  while (true) {
      ...
      case MAIN_MENU:
        handleMenu(MAIN_OPT, []() {
          switch (selected_option) {
            case 0: menu_state = PROFILE_MENU; break;
            case 1: menu_state = LOG_MENU;  break;
            case 2: menu_state = DISP_MENU;  break;
            case 3: menu_state = EXIT_MENU; break;
          }
        });
        break;
      ...
      case DISP_MENU:
        handleMenu(DISP_OPT, []() {
          if (selected_option < 2) {
            ...
            EEPROM.update(0, disp_out_mode);
            menu_state = MAIN_MENU;
          } else if (selected_option == 2) menu_state = LED_MENU;   // <-- this now works
            else menu_state = BCKLIGHT_MENU;
        });
        break;
      ...
      case LOG_MENU:
        handleMenu(LOG_OPT, []() {
          if (selected_option < 2) {
            ...
            EEPROM.update(1, log_flag);
            menu_state = MAIN_MENU;
          } else {
            flagEMU = (selected_option == 2) ? 1 : 0;
    ...
    if (menu_state == EXIT_MENU) break;
    // No forced "menu_state = MAIN_MENU" here.
    // handleMenu() already calls drawMenu() after each selection.
  }
...
void drawMenu() {
  switch (menu_state) {
    ...
    // --- Main Menu ---
    case MAIN_MENU: {
      const char* items[] = { "Profile", "Logging & Emu.", "Display", "Exit" };
    ...
    // --- Display Menu ---
    case DISP_MENU: {
      const char* items[] = { "BAR", "PSI", "LED Menu", "Backlight Menu" };
...
void sendProfileData(uint8_t data_sent) {
    uint8_t attempt = 0;
