...
uint16_t lambda_value;
uint16_t oxygen_value = 0; //currently unused                 
...
uint16_t lookup_oxygen(uint16_t input_adc) {
  
    //Declare and set default return value.
    uint16_t o2_content = 0;

    //Validate ADC range for lookup table.
    if (input_adc < 306) input_adc = 306;
    if (input_adc > 448) input_adc = 448;
    
    //Lookup Oxygen Concentration.
    o2_content = pgm_read_word_near(Oxygen_Conversion + (input_adc - 306));
...
void loop() {
  ...
  //oxygen_value = lookup_oxygen(sens_signal);
...
void sendCAN() {
    ...
    while (attempt < MAX_RETRIES) {
      if (CAN.beginPacket(0xFE, 8)) {
        ...
        CAN.write(highByte(oxygen_value));
        CAN.write(lowByte(oxygen_value));
