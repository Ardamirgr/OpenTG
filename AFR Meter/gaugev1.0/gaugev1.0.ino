...
uint8_t  vbat = 200;
uint16_t sens_temp, lambda_value, o2_value;
...
void setup() {
  ...
  ******************/
  ...
  TCCR0B = (1 << CS00);  //  WGM02 = 0, prescaler = 1
...
void loop(){
  ...
  if (status_flags & STATUS_NORMAL) {  
    ...
    uint32_t afr_int = afr / 100;      // Get the part before the decimal
    uint32_t afr_decimal = afr % 100;    // Get the part after the decimal
  
    UpdateLEDS(afr_int, led_mode);
    UpdateScreen(afr_int, afr_decimal);
...
void UpdateScreen(uint32_t afr_int, uint32_t afr_decimal) {
  ...
  // Determine AFR color
  if (afr > 1599) color = COLOR_RGB565_RED;
  else if (afr > 1399) color = COLOR_RGB565_YELLOW;
...
void errorCAN() {
  ...
  screen.println(" CAN com. failed!");
  screen.setTextSize(3);
  screen.println(" Check connector.");
  while(1) {    
    if (user_action == 0) user_action = flashLEDS();
    cli();
    wdt_reset(); 
...
void packetReceive(uint8_t packetSize) {
  ...
  if (CAN.packetId() == 0xFE && packetSize == 8) {
   ...
   // Read exactly 8 bytes (defensive against partial availability)
    ...
    o2_value     = (uint16_t)((uint16_t)buffCAN[3] << 8) | buffCAN[4];
...
/*void logData(int lambda_val, int o2_value) {

   if (status_flags == CJ125_DIAG_REG_STATUS_OK) {
      
      //Assembled data.
      txString = "Oxygen:";
      txString += String(o2_value, 2);
      txString += ",Lambda:";
